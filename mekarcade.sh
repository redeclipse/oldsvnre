...
if [ -x ${RE_PATH}/bin/mekclient_native ]
then
...
if [ -x ${RE_PATH}/bin/${RE_ARCH}mekclient${SYSTEM_SUFFIX} ]
then
    cd ${RE_PATH} || exit 1
    exec ${RE_PATH}/bin/${RE_ARCH}mekclient${SYSTEM_SUFFIX} ${RE_OPTIONS} "$@"
else
    ...
    if [ "${CC}" != "n" ]; then
        ...
        make -f Makefile.mek clean install
