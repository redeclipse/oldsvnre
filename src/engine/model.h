...
struct model
{
    ...
    virtual void render(int anim, int basetime, int basetime2, const vec &o, float yaw, float pitch, float roll, dynent *d, modelattach *a = NULL, const vec &color = vec(0, 0, 0), const vec &dir = vec(0, 0, 0), const vec &material1 = vec(1, 1, 1), const vec &material2 = vec(1, 1, 1), float transparent = 1, float size = 1) = 0;
    virtual bool load() = 0;
