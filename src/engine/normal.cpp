...
void show_addtnormals_nmprog()
{
    float bar1 = float(nmprog) / float(allocnodes);
    progress(bar1, "computing t-normals...");
}
...
{
    CHECK_CALCLIGHT_PROGRESS(return, show_addtnormals_nmprog);
...
void calcnormals(bool lerptjoints)
{
    ...
    if(lerptjoints) 
    {
        findtjoints();
        nmprog = 1;
        loopi(8) addtnormals(worldroot[i], ivec(i, 0, 0, 0, hdr.worldsize/2), hdr.worldsize/2);
    }
}
