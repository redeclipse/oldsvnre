...
- (NSString*)expand 
{
    ...
    [str setString:self];
    [str replaceOccurrencesOfString:@":s" withString:kredeclipse options:0 range:NSMakeRange(0, [str length])]; 
    return str;
}
@end
...
- (void)initPaths 
{
    ...
    // userpath: directory where user files are kept - typically /Users/<name>/Application Support/redeclipse
    FSRef folder;
    path = nil;
    if (FSFindFolder(kUserDomain, kApplicationSupportFolderType, NO, &folder) == noErr) 
    {
        ...
        path = [(NSURL *)url path];
        CFRelease(url);
        path = [path stringByAppendingPathComponent:kredeclipse];
        NSFileManager *fm = [NSFileManager defaultManager];
...
// plist registers 'redeclipse' as a url scheme
