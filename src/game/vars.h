...
#ifdef FPS // TODO: merge corectly with RE after 1.4
...
GFVAR(0, capturebuffarea, 0, 96, FVAR_MAX); // radius in which buffing occurs
...
GVAR(0, defendinterval, 0, 25, VAR_MAX);
GVAR(0, defendhold, 1, 200, VAR_MAX); // points needed to gain a score point
GVAR(0, defendoccupy, 1, 200, VAR_MAX); // points needed to occupy in regular games
...
GFVAR(0, defendbuffarea, 0, 64, FVAR_MAX); // radius in which buffing occurs
...
GVAR(0, zoomlock, 0, 1, 4); // 0 = unrestricted, 1 = must be on floor, 2 = also must not be moving, 3 = also must be on flat floor, 4 = must also be crouched
GVAR(0, zoomlocktime, 0, 250, VAR_MAX); // time before zoomlock kicks in when in the air
