                ...
                if(m_survivor(gamemode, mutators) && m_team(gamemode, mutators) && !alive.empty())
                {
                    ...
                    if(!found)
                    {
                        ...
                        else if(gamemillis-dueldeath > DEATHMILLIS)
                        {
                            if(!cleanup)
                            {
                                if(m_dm(gamemode)) srvmsgf(-1, "\fyteam \fs%s%s\fS are the victors", teamtype[alive[0]->team].chat, teamtype[alive[0]->team].name);
                                loopv(playing)
                                {
                ...
                else switch(alive.length())
                {
                    ...
                    case 1:
                    {
                        ...
                        else if(gamemillis-dueldeath > DEATHMILLIS)
                        {
                            if(!cleanup)
                            {
                                if(m_dm(gamemode)) srvmsgf(-1, "\fy%s was the victor", colorname(alive[0]));
                                loopv(playing)
                                {
