        ...
        switch(proj.projtype)
        {
            case PRJ_SHOT:
            {
                ...
                switch(proj.weap)
                {
                    ...
                    case WEAP_FLAMER:
                    {
                        ...
                        {
                            part_create(PART_FIREBALL_SOFT, max(int(projtraillength*0.5f*max(1.f-proj.lifespan, 0.1f)), 1), proj.o, firecols[0][rnd(FIRECOLOURS)], size, blend, -5);
                            proj.lasteffect = lastmillis - (lastmillis%(projtraildelay*2));
                        }
                        else part_create(PART_FIREBALL_SOFT, 1, proj.o, firecols[0][rnd(FIRECOLOURS)], size, blend);
                        break;
                    }
                    case WEAP_GRENADE:
                    {
                    ...
                    case WEAP_SHOTGUN:
                    {
                        ...
                        if(!proj.child && WEAP2(proj.weap, flakweap, proj.flags&HIT_ALT) >= 0)
                        {
                            ...
                            part_create(PART_HINT_SOFT, 1, proj.o, teamhint(proj.owner, col), WEAP2(proj.weap, partsize, proj.flags&HIT_ALT)*10.f*1.125f*proj.curscale, 0.5f);
                        }
                        else
                        {
                            float size = clamp(WEAP2(proj.weap, partlen, proj.flags&HIT_ALT)*(1.f-proj.lifespan)*proj.curscale, proj.curscale, min(min(WEAP2(proj.weap, partlen, proj.flags&HIT_ALT), proj.movement), proj.o.dist(proj.from)));
            ...
            case PRJ_GIBS: case PRJ_DEBRIS:
            {
                ...
                if(!proj.limited)
                {
                    ...
                    part_create(PART_FIREBALL_SOFT, len, proj.o, firecols[0][rnd(FIRECOLOURS)], radius, blend, -5);
                }
                break;
            }
            case PRJ_EJECT:
                        ...
                        if(notrayspam(proj.weap, proj.flags&HIT_ALT, 1))
                        {
                            ...
                            if(proj.weap != WEAP_FLAMER || proj.child || WEAP2(proj.weap, flakweap, proj.flags&HIT_ALT)%WEAP_MAX != WEAP_FLAMER)
                            {
                                loopi(proj.weap != WEAP_ROCKET ? 3 : 6)
                                {
                                    ...
                                    part_create(PART_FIREBALL_SOFT, projtraillength*(proj.weap != WEAP_ROCKET ? 2 : 3), to, firecols[0][rnd(FIRECOLOURS)], expl, 0.25f+(rnd(25)/100.f), -5);
                                }
                            }
