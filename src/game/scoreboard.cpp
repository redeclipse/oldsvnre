...
namespace hud
{
                    ...
                    if(m_fight(game::gamemode) && m_team(game::gamemode, game::mutators))
                    {
                        ...
                        else
                        {
                            if(numgroups > 1 && sg.total == groups[1]->total)
                            {
                                ...
                                game::announcef(S_V_DRAW, CON_MESG, game::player1, "\fw\fs\f[%d]%s\fS tied %swith a total score of: \fs\fc%d\fS", TEAM(sg.team, colour), TEAM(sg.team, name), winner, sg.total);
                            }
                    ...
                    else
                    {
                        ...
                        if(m_trial(game::gamemode))
                        {
                            if(sg.players.length() > 1 && sg.players[0]->cptime == sg.players[1]->cptime)
                            {
                                ...
                                game::announcef(S_V_DRAW, CON_MESG, game::player1, "\fw%s tied %swith the fastest lap: \fs\fc%s\fS", game::colorname(sg.players[0]), winner, sg.players[0]->cptime ? timetostr(sg.players[0]->cptime) : "dnf");
                            }
                        ...
                        else
                        {
                            if(sg.players.length() > 1 && sg.players[0]->points == sg.players[1]->points)
                            {
                                ...
                                game::announcef(S_V_DRAW, CON_MESG, game::player1, "\fw%s tied %swith a total score of: \fs\fc%d\fS", game::colorname(sg.players[0]), winner, sg.players[0]->points);
                            }
    ...
    int drawscoreitem(const char *icon, int colour, int x, int y, int s, float skew, float fade, int pos, int score, const char *name)
    {
        ...
        if(m_defend(game::gamemode) && score == INT_MAX)
            hud::drawitem(icon, x, y+size, s, inventoryscorebg!=0, 0, false, c.r, c.g, c.b, fade, skew, "huge", "%sWIN", col);
        else hud::drawitem(icon, x, y+size, s, inventoryscorebg!=0, 0, false, c.r, c.g, c.b, fade, skew, "huge", "%s%d", col, score);
        hud::drawitemtext(x, y+size, 0, false, skew, "default", fade, "\f[%d]%s", colour, name);
        return size;
    }
        ...
        loopi(2) loopk(numgroups)
        {
            ...
            else
            {
                ...
                loopvj(sg.players)
                {
                    ...
                    float sk = numout && inventoryscoreshrink > 0 ? 1.f-min(numout*inventoryscoreshrink, inventoryscoreshrinkmax) : 1;
                    sy += drawscoreitem(hud::playertex, game::getcolour(d, game::playerdisplaytone), x, y+sy, s, sk*inventoryscoresize, blend*inventoryblend, j, d->points, game::colorname(d, NULL, "", false));
                    if(++numout >= inventoryscore) return sy;
                }
            }
        }
